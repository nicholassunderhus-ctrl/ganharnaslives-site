...
import { useUserPoints } from "@/hooks/useUserPoints"; // Hook para buscar os pontos do usuário
...
const Dashboard = () => {
  ...
  // Se o usuário está no /dashboard, mostra o conteúdo principal.
  // Se estiver em uma sub-rota (ex: /dashboard/watch), o <Outlet> renderizará o componente daquela rota.
  const isDashboardHome = location.pathname === "/dashboard";

  const MainContent = () => (
    <div className="max-w-7xl mx-auto space-y-8">
      {isDashboardHome ? <DashboardHome stats={stats} /> : <Outlet />}
    </div>
  );
...
// Componente para a página inicial do dashboard, separado para clareza.
const DashboardHome = ({ stats }: { stats: { points: number; totalEarned: number } }) => {
  const pointsToReal = (points: number) => (points / 1400).toFixed(2);
